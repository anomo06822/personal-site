...
import { getProjectHref, withBasePath } from "@/lib/site";
import type {
  Locale,
  PersonalProjectItem,
...
function PersonalShowcaseCard({
  ...
  return (
    <Link
      ...
      className="card-surface group block overflow-hidden transition hover:-translate-y-1 hover:border-accent/50 hover:bg-panel-strong"
    >
      <ShowcasePreview
        item={item}
        tabLabel={tabLabel}
      ...
      />
      <ShowcaseCopy item={item} />
      <div className="px-6 pb-6 sm:px-7 sm:pb-7">
        <div className="inline-flex min-h-11 items-center rounded-full border border-accent/35 bg-[var(--accent-soft)] px-5 text-sm text-ink transition group-hover:border-accent/60 group-hover:bg-panel-strong">
          {personalCtaLabel}
        </div>
      </div>
    </Link>
            ...
            tab === "personal" ? (
              <div className="grid gap-4 xl:grid-cols-3">
                {items.personal.map((item) => (
                  <PersonalShowcaseCard
                    key={item.slug}
                    item={item}
                    locale={locale}
                    tabLabel={tabs.personal}
                    imageSlotLabel={imageSlotLabel}
                    personalCtaLabel={personalCtaLabel}
                  />
                ))}
              </div>
            ) : (
